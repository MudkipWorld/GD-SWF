...
public partial class GenScript : Node {
    ...
    private Dictionary<int, string> symbolNames = new();
    ...
    public Godot.Collections.Dictionary LoadSwf(string path, bool baked_data = true)
    {
        ...
        foreach (var kvp in spriteDict)
        {
            ...
            var spriteTag = kvp.Value; // THIS is the sprite being processed
            exportDoc.Sprites[kvp.Key] = ProcessTimeline(spriteTag.Tags, baked_data, spriteTag.SpriteID);
        }
    ...
    private void BuildDefinitionDictionaries(){
        ...
        void RecurseDefinitions(IEnumerable<SwfTagBase> tags)
        {
            foreach (var tag in tags)
            {
                switch (tag)
                {
                    case SymbolClassTag symbolClass:
                        foreach (var reference in symbolClass.References)
                            symbolNames[reference.SymbolID] = reference.SymbolName;
                        break;
    ...
    private SpriteExportData ProcessTimeline(IEnumerable<SwfTagBase> tags, bool bakeFrames = true, int? spriteID = null){
        ...
        var sprite_name = "";
        if (spriteID.HasValue && symbolNames.TryGetValue(spriteID.Value, out var n))
            sprite_name = n;
    ...
    private void UpdateDisplayObject( Dictionary<int, FrameTag> displayList, List<ChildInfo> childrenList,int characterId,int depth, SwfMatrix matrix,bool hasCharacter,bool hasMatrix,object colorTransform = null){
    ...
    public class SpriteExportData { public List<ChildInfo> Children = new(); public List<Dictionary<int, FrameTag>> Frames = new(); public List<string> FrameNames = new(); public float LocalX, LocalY;  public int MaxNestingDepth = 0; public Dictionary<int, FrameTag> FirstFrameData = new(); public String SpriteName = "";}
